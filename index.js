...
const vcard = 'BEGIN:VCARD\n' // metadata of the contact card
            ...
            + 'TEL;type=CELL;type=VOICE;waid=:+62 895-0993-8750\n' // WhatsApp ID + phone number
            + 'END:VCARD'
prefix = '#'
blocked = []

const config = {
        botname: 'Bot Cantik', // Your Name Bot
        oname: 'Style Cogan' // Your Name
}
...
async function starts() {
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Sayonara @${num.split('@')[0]}semoga tenang dialam sana👋`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			const ownerNumber = ["089509938750@s.whatsapp.net"] // replace this with your number
